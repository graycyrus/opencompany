...
> **Re-checked 2026-08-21 — still true.** The occurrence count and line numbers
> have moved (`src/company/manifest.rs:907` is now the display path), but
> `human_role` still has zero references anywhere under `src/policy/`, and
> `validate()` (`src/company/manifest.rs:310`) still does not enforce the lint
...
> **Correction, 2026-08-21.** Conditions 1 and 2 have since been met. A dedicated
> `Rust (openhuman, tinycortex)` CI lane builds, clippies and tests the owned
> runtime, and `src/harness/built_in/build.rs:289` hands agents a real,
> per-agent-narrowed tool vector. Condition 3 — measured and published
> reliability — is still unmet, which keeps the conclusion of this section
> intact: the moat remains unproven, but it is no longer blocked on the first two
> preconditions. See
> [README.md — Corrections](README.md#corrections-verified-2026-08-21).
