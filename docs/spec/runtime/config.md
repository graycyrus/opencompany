...
| `OPENCOMPANY_INFERENCE_MODEL` | `chat-v1` | Roster-wide default model/tier for the harness brain (`openhuman` feature). A managed turn honours it only when it is a real OpenRouter id — a tier is refused, not guessed at. It is not how a company chooses Managed's model: that is Managed's model on the Providers tab, stored per company in `inference/managed/models` (issue #2303) |
