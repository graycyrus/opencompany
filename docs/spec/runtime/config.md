...
| `OPENCOMPANY_INFERENCE_URL` | `https://api.tinyhumans.ai/openai/v1` | Harness-brain OpenAI-compatible endpoint (`openhuman` feature). Managed company turns and the managed catalog use its **origin** on the OpenRouter proxy path, `/agent-integrations/openrouter` — see [the managed endpoint](providers.md#the-managed-endpoint-issue-2303) |
| `OPENCOMPANY_INFERENCE_MODEL` | `chat-v1` | Roster-wide default model/tier for the harness brain (`openhuman` feature). A managed turn honours it only when it is a real OpenRouter id — a tier is refused, not guessed at. It is an instance-wide fallback used by test and fixture hosts, **not** how a company chooses its managed model; that per-company choice is not built yet (issue #2303) |
