...
| [runtime/analytics-wire.md](runtime/analytics-wire.md) | The OpenPanel `POST /track` contract that transport speaks: the two auth headers, the union body, the names the collector refuses, where an event time goes, and what a drain does when the collector will not answer |
