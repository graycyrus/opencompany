...
/// Embeds every shipped workflow graph — `companies/<id>/workflows/*.toml` and
/// `globals/workflows/*.toml` — into `shipped_workflows.rs`, keyed by
/// repo-relative path.
///
/// Globbed at build time rather than hand-listed for the reason the roster is:
/// a workflow added to a bundle is covered because it exists, not because
/// somebody remembered to add a line to a test. Forty-five graphs ship as of
/// this commit, and before it exactly **two** of them were asserted against
/// anywhere — so a seed that no longer parsed, translated or compiled would
/// have reached a customer's first run with nothing red in between.
///
/// **Fail-loud, unlike [`embed_globals`].** A missing `companies/` is still not
/// this script's business (a source-only build has no bundles), but if
/// `companies/` IS present then this is a real checkout and an absent
/// `globals/workflows/` or an empty harvest is an accidental deletion, not a
/// packaging mode. Emitting an empty table there would leave the corpus suite
/// iterating nothing and reporting green — the silent-skip failure the suite
/// exists to prevent, one level lower down.
fn embed_shipped_workflows(root: &Path) {
    let companies = root.join("companies");
    let globals_workflows = root.join("globals").join("workflows");
    println!("cargo:rerun-if-changed={}", globals_workflows.display());

    let mut files: Vec<(String, String)> = Vec::new();

    if !companies.is_dir() {
        write_shipped_workflows(&files);
        return;
    }

    if let Ok(entries) = std::fs::read_dir(&companies) {
        for entry in entries.filter_map(Result::ok) {
            let dir = entry.path().join("workflows");
            if !dir.is_dir() {
                continue;
            }
            println!("cargo:rerun-if-changed={}", dir.display());
            collect_workflow_tomls(root, &dir, &mut files);
        }
    }

    assert!(
        globals_workflows.is_dir(),
        "`companies/` is present but `{}` is missing — the global baseline's workflows cannot be \
         checked. Restore the directory rather than shipping a corpus suite that silently covers \
         nothing.",
        globals_workflows.display()
    );
    collect_workflow_tomls(root, &globals_workflows, &mut files);

    assert!(
        !files.is_empty(),
        "no `workflows/*.toml` found under `{}` or `{}` — an empty corpus suite reports green \
         while checking nothing.",
        companies.display(),
        globals_workflows.display()
    );

    // Sorted so the generated file is byte-stable across builds and hosts;
    // `read_dir` order is neither.
    files.sort();
    write_shipped_workflows(&files);
}

/// Every `*.toml` directly inside `dir`, keyed by its path relative to `root`
/// with forward slashes, so a failure names the file an author would open.
///
/// Deliberately not recursive: `workflows/` is a flat directory of graphs, and
/// the loader (`load_company_workflows`) only ever resolves `<id>.toml` at its
/// top level, so a nested file is not a shipped workflow.
fn collect_workflow_tomls(root: &Path, dir: &Path, out: &mut Vec<(String, String)>) {
    let Ok(entries) = std::fs::read_dir(dir) else {
        return;
    };
    for entry in entries.filter_map(Result::ok) {
...
fn write_shipped_workflows(files: &[(String, String)]) {
    let mut out = String::from("// @generated by build.rs — do not edit.\n");
    write_pairs(
        &mut out,
        "SHIPPED_WORKFLOWS",
        "Every shipped workflow graph, keyed by repo-relative path, sorted.",
        files,
    );
    let dest = PathBuf::from(std::env::var("OUT_DIR").unwrap()).join("shipped_workflows.rs");
    std::fs::write(dest, out).unwrap();
}
