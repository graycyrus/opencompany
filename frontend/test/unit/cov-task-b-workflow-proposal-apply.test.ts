...
describe("Apply a automation proposal — offered to every member, not just an admin", () => {
...
describe("Apply a automation proposal — the host refuses it", () => {
  it("keeps the card in review, shows the host's reason, and never claims success", async () => {
    const post = vi.fn(async () => {
      throw new ApiError(400, "conflict", "a automation named “weekly_summary” already exists", true);
    });
    const onReload = await render(clientAs(post));
    ...
    expect(error?.textContent).toContain(
      "a automation named “weekly_summary” already exists",
    );
