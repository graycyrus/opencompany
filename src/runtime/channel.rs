...
pub fn undeliverable_channel_message(target: &str, deliverable: &[&str]) -> String {
    ...
    // "automation", not "workflow": the console renamed the surface an author
    // reads this on, and the guard test below exists precisely so the two
    // cannot say different words for the same refusal. The module is still
    // `workflows` internally, which is a different audience.
