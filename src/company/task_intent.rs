...
///
/// # The one verdict that has to be the whole sentence
///
...
/// left paused with nothing behind it to restart it, and the reply was two
/// words naming nothing.
///
...
/// is [`Amend`](BlockerReplyIntent::Amend), the answer it was written to give.
///
/// **Only cancel**, and the asymmetry is the reason. Cancel is the one verdict
/// that does not re-enter: being wrong about it destroys the work, while being
/// wrong about retry, skip or amend costs a re-run the next message can
/// correct. It is the same trade [`triage_message`] already makes between
/// [`Answer`](MessageTriage::Answer) and [`Chatter`](MessageTriage::Chatter),
/// for the same reason — only the expensive direction is worth a guard.
...
/// Whether abandoning the work is the **whole** of what `lower` asks for — every
/// clause that does not itself carry a [`CANCEL_WORDS`] entry is filler.
///
/// Clause-wise rather than message-wise, over the same [`clauses`] split
/// [`later_clause_is_imperative`] uses, so the two places that ask "does a later
/// part of this sentence say something the first part does not" answer it the
/// same way. A comma is deliberately *not* a clause boundary there, which keeps
/// "cancel this one, retry the other" a cancel — that reading is the priority
/// rule this function is scoped not to disturb.
///
...
/// nothing else.
///
/// # A cancel clause is exempt, not invisible
///
...
/// clause, and a blanket exemption throws the budget away with it: the message
/// reads as a pure cancel and the amendment never reaches anywhere.
///
/// A clause is exempt only when it carries no digit. A cancel word beside
/// ordinary filler — "it", "this one", "that", pronouns [`is_pure_social`] does
/// not even recognise — stays exempt exactly as before, which is what keeps
/// every existing case unchanged: none of them names a number. A digit is the
/// cheap, reliable tell that a clause carries a genuine second fact rather than
/// filler around the cancel word — an amount, a count — and once kept in,
/// [`is_pure_social`] correctly fails it (digits are not [`SOCIAL_WORDS`]), so
...
mod blocker_reply_tests {
    ...
    /// A message whose main content is the answer is not a cancel because one
    /// clause of it mentions scrapping something else.
    ///
    /// This exact sentence cancelled a card and threw the answer away with it.
    /// The card was left paused with no blocker behind it, so nothing would ever
    /// have resumed it, and the reply named nothing.
    #[test]
    ...
    /// The guard is a scope, not a removal: a cancel that really is the whole of
    /// the ask still cancels, filler and all.
    #[test]
    ...
    /// A cancel word beside a genuine second fact must not swallow it. A comma
    /// is not a clause boundary (`cancel_is_the_whole_ask`'s own doc), so "scrap
    /// the reminder card, budget is $500" is one clause naming both the cancel
    /// and the amount — discarding it wholesale because it names the cancel lost
    /// the $500 entirely and settled the blocker as a pure cancel nobody asked
    /// for.
    #[test]
