...
impl CompanyRuntime {
    ...
    /// Gives a parked approval another full TTL window, so the operator has more
    /// time before it default-denies (issue #1805).
    ///
    ...
    ///
    /// # It adds a window; it does not re-anchor to now
    ///
    ...
    /// answered `200 {"extended":true}` and the countdown read the same before
    /// and after. The whole argument is in
    /// [`ManifestApprovalGate::extend`](crate::policy::gate::ManifestApprovalGate::extend),
